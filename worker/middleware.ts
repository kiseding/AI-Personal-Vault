 ...
 *  - 去掉「vault 已初始化 → 跳过 APP_TOKEN」逻辑。
 *    即便用户已解锁主密码，所有非白名单 /api/* 都仍要求 Bearer APP_TOKEN。
 *    防止 URL 泄漏后攻击者无需任何凭证就能列出 entry 密文。
 */
 ...
 * 访问控制中间件。
 *
 * 鉴权策略（feat/auth-hardening 后）：
 *  - 白名单放行（无需 APP_TOKEN）：
 *      · GET /api/ai/fetch/:id        X-AI-Token 鉴权（见 routes/ai.ts）
 *      · GET /api/ai/share/:id        提取码鉴权（见 routes/ai.ts）
 *      · GET /api/vault/config        让 Auth 屏能查询 setup 状态决定 UI
 *      · POST /api/vault/setup        首次初始化需要
 *  - 其余 /api/*：必须携带 Authorization: Bearer <APP_TOKEN>
 *
 * 与上一版的差别：移除了「vault:setup 已设置 → 跳过 token 校验」分支。
 * 该分支原本为了"用户已用主密码自证身份"的便利，但留下了"URL 泄露 → 任意人尝试密码"
 * 这个攻击面。Auth 流程同时把 token 写入 localStorage，正常用户体验不变。
 */
export const authMiddleware = createMiddleware<AppContext>(async (c, next) => {
  // 1. AI 读取端点放行（用各自 token 鉴权）
  if (
  ...
  // 2. config / setup 接口放行（首次初始化流程 / 解锁屏拉取 salt+verifier）
  if (
